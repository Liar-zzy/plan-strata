...
新会话可从 `plans/CURRENT.md` 恢复所需计划上下文，再读取下一项所需的产物及证据；
已有可靠上下文不必每轮重读。对于已开始的任务，使用 progress 中的版本化 `plan`
路径，即使 CURRENT 后来选择了别的版本也要先处理原绑定。
演示项目的剩余 T02 是写 `docs/usage.md`，不要与本仓库自身的开发任务混淆。

## 在自己的项目中开始或恢复

优先复用项目原有任务卡、PR/CI 证据与进度记录，不另建重复台账。需要 bundled
格式时才从 `assets/` 取计划模板；项目目的与关键约束未变可沿用 core。填写目标、
适用的资源边界、任务输入输出、依赖和验收方法；实现细节不必提前固定。
新计划默认 `ready: false`；完成定义后才能显式选择为新工作入口，选择本身不授权执行。

采用 bundled 格式时，记录分工与验收约定如下：

1. core 固定项目目的与关键边界；执行计划固定任务与验收并引用 core 路径。
   选定 ready 计划后，一次更新 CURRENT 的 `plan` 和 `progress` 路径。
2. progress 是任务实时状态的唯一位置，由项目指定 writer 维护负责人、绑定和下一步。
   已有宿主适配负责独占分配时，可在返回 Agent ID 后补记分配回执。
3. 执行实际验收方法，保存日志或具体审阅观察；check 用路径列表记录产物与证据。
   check 需写明检查者、日期、方法、实际结果、限制和下一步，不能引用自身作证据。
4. 检查通过且依赖完成后，由获准 Manager 接受、指定 writer 记录 `done`。需要集成
   时验证最终组合状态，可复用现有 CI，无需独立 Agent 或工作目录；科研检查另区分
   活动有效性、研究发现与后续决定。只读审阅者可在回复中给结论，不必写 check 文件。
5. 可运行 `validate --project <目标项目目录>` 核对记录。先报告问题，仅修复获准写入的
   记录；bundled 验收仍需要指定 writer 能引用的实际 check 文件。当前所有任务完成，且
...
[协议](../skills/plan-strata/references/protocol.md)；验收遵循
[验收约定](../skills/plan-strata/references/protocol.md#acceptance)和
[科研说明](../skills/plan-strata/references/research.md)。

已使用的 core 和计划保持不变；改变已约定范围、验收、接口依赖、资源边界或固定
科研方法时新建修订，明确切换入口并判断旧结果的适用性。普通实现、调试和测试方式
可在承诺内调整，不另修计划。相关代码、测试、数据或配置变化后，由指定 writer
